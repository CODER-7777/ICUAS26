...
struct DroneContext {
  ...
  rclcpp::Publisher<icuas25_msgs::msg::TargetInfo>::SharedPtr target_pub;
};
...
struct MarkerAggregator {
  ...
  size_t last_published_count; // Track how many detections were used in last
                               // publish
...
public:
  MultiArucoDetector() : Node("multi_aruco_detector") {
    ...
    // Global target publisher
    global_target_pub_ = this->create_publisher<icuas25_msgs::msg::TargetInfo>(
        "/target_node", 10);
    ...
    for (const auto &cf : cf_ids_) {
      ...
      ctx->target_pub = this->create_publisher<icuas25_msgs::msg::TargetInfo>(
          std::string("/") + cf + "/target_found", 10);
    ...
    RCLCPP_INFO(this->get_logger(),
                "Multi-Drone ArUco Detector Started (C++) - Averaging Enabled");
    RCLCPP_INFO(this->get_logger(),
                "Detection timeout: %.1fs, Outlier threshold: %.1f sigma",
                detection_timeout_, outlier_threshold_);
  }
...
private:
  ...
  void imageCb(const sensor_msgs::msg::Image::SharedPtr msg,
    ...
    RCLCPP_INFO(this->get_logger(), "[%s] Detected %zu ArUco marker(s)",
                cf.c_str(), ids.size());
    ...
    for (size_t i = 0; i < ids.size(); ++i) {
      int mid = ids[i];
      if (ctx->marker_seen.count(mid)) {
        RCLCPP_DEBUG(this->get_logger(), "[%s] Marker %d already seen",
                     cf.c_str(), mid);
        continue;
      }
      ctx->marker_seen.insert(mid);
      RCLCPP_INFO(this->get_logger(), "[%s] 🎯 NEW marker %d detected!",
                  cf.c_str(), mid);
  ...
  void publishTarget(const std::string &cf, int marker_id, double tx, double ty,
    ...
    // Individual detection logged but not published yet
    RCLCPP_DEBUG(this->get_logger(),
                 "[%s] Detected Marker %d @ (%.2f, %.2f, %.2f) dist=%.2fm",
                 cf.c_str(), marker_id, world_x, world_y, world_z, distance);
  }
  ...
  void storeDetection(int marker_id, const std::string &drone_id, double x,
    ...
    // Check if this drone already detected this marker
    for (const auto &det : agg.detections) {
      if (det.drone_id == drone_id) {
        return; // Already have detection from this drone
      }
    }
    ...
    if (agg.detections.size() == 1) {
      ...
      RCLCPP_INFO(
          this->get_logger(),
          "⏱️  Marker %d: First detection by %s - starting %.1fs timeout",
          marker_id, drone_id.c_str(), detection_timeout_);
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "📊 Marker %d: Now detected by %zu drones (latest: %s)",
                  marker_id, agg.detections.size(), drone_id.c_str());

      // Check if timeout already passed - if so, re-average immediately
      double elapsed = (this->now() - agg.first_detection_time).seconds();
      if (elapsed > detection_timeout_) {
        RCLCPP_INFO(
            this->get_logger(),
            "🔄 Marker %d: Late detection! Re-averaging with %zu drones",
            marker_id, agg.detections.size());
  ...
  void checkAndPublishPendingMarkers() {
    ...
    for (auto &[marker_id, agg] : marker_detections_) {
      ...
      // Only publish if we have NEW detections (more than last time)
      if (agg.detections.size() <= agg.last_published_count)
        continue;

      double elapsed = (now - agg.first_detection_time).seconds();

      // Publish if timeout reached (first time or re-averaging)
      if (elapsed > detection_timeout_) {
        if (agg.last_published_count == 0) {
          RCLCPP_INFO(this->get_logger(),
                      "⏰ Marker %d: Timeout reached (%.1fs) - publishing with "
                      "%zu drones",
                      marker_id, elapsed, agg.detections.size());
        } else {
          RCLCPP_INFO(this->get_logger(),
                      "🔄 Marker %d: Re-publishing with %zu drones (was %zu)",
                      marker_id, agg.detections.size(),
                      agg.last_published_count);
        }
        publishAveragedMarker(marker_id, agg);
      }
      // Or publish immediately if all drones detected
      else if (agg.detections.size() >= cf_ids_.size()) {
        RCLCPP_INFO(this->get_logger(),
                    "✅ Marker %d: All %zu drones detected - publishing now!",
                    marker_id, agg.detections.size());
        publishAveragedMarker(marker_id, agg);
      }
    }
  ...
  void publishAveragedMarker(int marker_id, MarkerAggregator &agg) {
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "\u2699\ufe0f  Averaging coordinates for Marker %d from %zu drones...",
        marker_id, agg.detections.size());
    ...
    std::vector<std::string> included_drones;
    std::vector<std::string> excluded_drones;
    ...
    for (const auto &det : agg.detections) {
      ...
      if (n > 2 && deviation > outlier_threshold_ * std_dev) {
        excluded_drones.push_back(det.drone_id);
        continue;
      }
      ...
      total_weight += weight;
      included_drones.push_back(det.drone_id);
    }
    ...
    if (gt_it != GROUND_TRUTH.end()) {
      ...
      double dist = std::sqrt((avg_x - gt_x) * (avg_x - gt_x) +
                              (avg_y - gt_y) * (avg_y - gt_y) +
                              (avg_z - gt_z) * (avg_z - gt_z));
      err_msg =
          " | Error: " + std::to_string(dist * 100.0).substr(0, 6) + " cm";
    } else {
      err_msg = " | Error: GT Not Found";
    }

    // Publish averaged result
    icuas25_msgs::msg::TargetInfo msg;
    msg.id = marker_id;
    msg.location.x = static_cast<float>(avg_x);
    msg.location.y = static_cast<float>(avg_y);
    msg.location.z = static_cast<float>(avg_z);

    global_target_pub_->publish(msg);

    // Also publish to individual drone topics for compatibility
    for (const auto &drone_id : included_drones) {
      auto it = drones_.find(drone_id);
      if (it != drones_.end()) {
        it->second->target_pub->publish(msg);
      }
    }

    // Update last published count (allows re-publishing with more drones)
    agg.last_published_count = included_drones.size();

    // Log result
    std::string drones_str;
    for (size_t i = 0; i < included_drones.size(); ++i) {
      if (i > 0)
        drones_str += ",";
      drones_str += included_drones[i];
    }

    std::string log_msg =
        "[AVERAGED " + std::to_string(included_drones.size()) +
        " drones: " + drones_str + "] Marker " + std::to_string(marker_id) +
        " @ (" + std::to_string(avg_x).substr(0, 5) + ", " +
        std::to_string(avg_y).substr(0, 5) + ", " +
        std::to_string(avg_z).substr(0, 5) + ")" + err_msg;

    if (!excluded_drones.empty()) {
      log_msg += " | Outliers rejected: ";
      for (size_t i = 0; i < excluded_drones.size(); ++i) {
        if (i > 0)
          log_msg += ",";
        log_msg += excluded_drones[i];
      }
    }
  ...
  rclcpp::Publisher<icuas25_msgs::msg::TargetInfo>::SharedPtr
      global_target_pub_;
