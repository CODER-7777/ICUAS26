...
private:
    ...
    void timerLoop() {
        ...
        if (!path.empty()) {
            ...
            RCLCPP_WARN(this->get_logger(), "No possible path");
        }
    ...
    void fetchOctomapOnce() {
        ...
        if (future.wait_for(3s) != std::future_status::ready) {
            RCLCPP_ERROR(this->get_logger(), "Failed to get OctoMap (timeout)");
            return;
        } else {
            RCLCPP_ERROR(this->get_logger(), "Octomap request received");
        }
        ...
        if (!abs_tree) { return; } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to get Abstract Octree from octomap msg");
        }

        tree_.reset(dynamic_cast<octomap::OcTree *>(abs_tree));
        if (!tree_) { return; } else {
            RCLCPP_ERROR(this->get_logger(), "Octree type is wrong");
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Map z_max=%.2f m: initial z_target=%.2f, z_step=%.2f", z_max, initial_z,
                    z_step_.load());
    ...
    std::vector<idx> bfs(idx start, idx end, const nav_msgs::msg::OccupancyGrid &map) {
        ...
        bool found = false;
        while (!q.empty()) {
            int curr_flat = q.front();
            q.pop();
            if (curr_flat == end_idx) {
                found = true;
                break;
            }
