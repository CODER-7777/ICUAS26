...
public:
    OctomapBFSPlanner() : Node("octomap_bfs_planner") {
        ...
        is_planning_ = false;
        RCLCPP_INFO(this->get_logger(), "Planner initialized and awaiting AGV pose.");
    }
...
private:
    ...
    void timerLoop() {
        ...
        if (elapsed > 120.0) {
            RCLCPP_INFO_ONCE(this->get_logger(), "120s run time limit reached.");
            return;
        }
        
        // Prevent concurrent execution of the planning pipeline
        if (is_planning_) return;

        if (!has_pose_ || !client_->service_is_ready()) return;
    ...
    void processPlanning(const octomap_msgs::msg::Octomap& msg, double timestamp) {
        ...
        // Use a unique_ptr for automatic memory management of the converted map
        auto tree = std::unique_ptr<octomap::OcTree>(dynamic_cast<octomap::OcTree*>(abs_tree));
        if (!tree) return;

        // Shared ownership for functions within the pipeline
        auto tree_shared = std::shared_ptr<octomap::OcTree>(std::move(tree));

        auto grid = generateOccupancyGrid(tree_shared);
        ...
        auto path = runPlanningPipeline(grid, target, tree_shared);

        RCLCPP_INFO(this->get_logger(), "[T=%.1f] Calculated %zu Drone waypoints", timestamp, path.size());
    ...
    std::vector<geometry_msgs::msg::Point> runPlanningPipeline(const nav_msgs::msg::OccupancyGrid& map, 
                                                              geometry_msgs::msg::Point target,
        ...
        // Greedy Waypoint Generation with Line of Sight
        while (rclcpp::ok() && !(cur.i == end.i && cur.j == end.j)) {
            ...
            for (int i = (int)grid_path.size() - 1; i >= 0; --i) {
                ...
                if (d > 0.01 && d <= maxD && hasLineOfSight(p1, p2, tree)) {
                    cur = grid_path[i]; 
                    drone_pts.push_back(p2); 
                    moved = true; 
                    break;
                }
            }
    ...
    std::vector<idx> bfs(idx start, idx end, const nav_msgs::msg::OccupancyGrid& map) {
        ...
        std::vector<idx> path;
        for (idx c = end; c.i != -1; c = parent[c.j * w + c.i]) {
            path.push_back(c);
        }
    ...
    bool hasLineOfSight(const geometry_msgs::msg::Point& a, const geometry_msgs::msg::Point& b, std::shared_ptr<octomap::OcTree> tree) {
        octomap::point3d origin(a.x, a.y, a.z);
        octomap::point3d end_p(b.x, b.y, b.z);
        octomap::point3d hit;
        return !tree->castRay(origin, (end_p - origin).normalize(), hit, true, origin.distance(end_p));
    }

    nav_msgs::msg::OccupancyGrid generateOccupancyGrid(std::shared_ptr<octomap::OcTree> tree) {
        ...
        // Iterate only through the leaf nodes within the target height slice
