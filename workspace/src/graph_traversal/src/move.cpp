...
public:
    SwarmPlanner() : Node("Swarm_planner") {
        ...
        control_timer_ = this->create_wall_timer(50ms, std::bind(&SwarmPlanner::publishCommands, this),
                                                 callback_group_);
...
private:
    ...
    void initializeBMS() {
        ...
        geometry_msgs::msg::Point s0;
        s0.x = cx - 0.8;
        s0.y = cy - 0.8;
        s0.z = 0.0;
        geometry_msgs::msg::Point s1;
        s1.x = cx + 0.8;
        s1.y = cy + 0.8;
        s1.z = 0.0;
    ...
    void fetchOctomapOnce() {
        ...
        octomap_client_->async_send_request(req,
                                            [this](rclcpp::Client<octomap_msgs::srv::GetOctomap>::SharedFuture future) {
                                                auto res = future.get();
                                                octomap::AbstractOcTree *abs_tree = octomap_msgs::binaryMsgToMap(
                                                    res->map);
                                                if (abs_tree) {
                                                    std::lock_guard<std::mutex> lock(data_mutex_);
                                                    ...
                                                    RCLCPP_INFO(this->get_logger(),
                                                                "Map Received. Grid Resolution: %.2f",
    ...
    std::vector<idx> bfs(idx start, idx end, const nav_msgs::msg::OccupancyGrid &map) {
        ...
        if (start.i < 0 || start.i >= w || start.j < 0 || start.j >= h ||
            end.i < 0 || end.i >= w || end.j < 0 || end.j >= h)
            return {};
        ...
        while (!open_set.empty()) {
            ...
            // Lazy skipping
            if (visited_token[curr_idx] == current_token && g_score[curr_idx] < current.f_score - std::max(
                    std::abs(end.i - (curr_idx % w)), std::abs(end.j - (curr_idx / w)))) {
                continue;
            }
    ...
    std::vector<geometry_msgs::msg::Point> runPlanningPipeline(const nav_msgs::msg::OccupancyGrid &map,
                                                               ...
                                                               geometry_msgs::msg::Point current_pos, double target_z) {
    ...
    void handleBatteryLogic(const std::string &id) {
        ...
        if (state.is_going_to_charge) {
            ...
            if (dist < 0.1 && !state.is_charging) {
                RCLCPP_INFO(this->get_logger(), "Drone %s arrived at charging slot %d. 'Landing' to Z=0.05.",
                            id.c_str(), state.assigned_slot);
    ...
    void handleMission() {
        ...
        const double z_mission = has_z_target_
                                     ? z_target_from_bfs_.load()
                                     : this->get_parameter("z_target").as_double();
...

        {
            ...
            for (const auto &id: droneIds_) {
                ...
                if (battery_states_[id].is_charging || battery_states_[id].is_going_to_charge || battery_states_[id].
                    is_leaving_charger) {
                    ...
                    if (battery_states_[id].percentage < CHARGING_THRESHOLD && !battery_states_[id].has_charged) {
                        ...
                        if (currently_charging_count < MAX_CHARGING_DRONES && free_slot != -1) {
                            ...
                            RCLCPP_WARN(this->get_logger(),
                                        " Drone %s Low Battery (%.1f%%). Sending to Charge Slot %d.",
                ...
                if (dist_from_center > 1.5 || elapsed > 5.0) {
                    // Finished clearing
                    {
                        ...
                        if (s >= 0 && s < (int) slot_occupied_.size()) {
                            ...
                            RCLCPP_INFO(this->get_logger(), " Slot %d freed by %s (Cleared dist=%.2fm)", s, id.c_str(),
                                        dist_from_center);
                        }
        ...
        std::vector<std::pair<std::string, std::vector<crazyflie_interfaces::msg::Position> > > new_commands(
            droneIds_.size());
            ...
            if (!job.is_assigned && dist_to_target < 0.2) {
                ...
                if (!path.empty()) {
                    for (size_t k = 0; k < path.size(); ++k) {
                        ...
                        double dx, dy;
                        if (k == 0) {
                            dx = path[k].x - current_pos.x;
                            dy = path[k].y - current_pos.y;
                        } else {
...
#pragma omp critical
        ...
        if (rth_index_ >= (int) droneIds_.size()) {
            if (!landing_service_called_) {
                RCLCPP_INFO(this->get_logger(),
                            "All drones at home position (Z=0.05). Triggering Final Land Service for ALL.");
